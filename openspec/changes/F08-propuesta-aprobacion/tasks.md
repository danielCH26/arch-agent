...
- [ ] 9.1 **Vitest for `CitationList`** Â· slice_2_sse_and_frontend Â· NEW `frontend/src/components/proposals/__tests__/CitationList.test.tsx` Â· 25 LoC tests Â· SCN-2, SCN-3 Â· `cd frontend && npm run test:run -- CitationList` passes Â· commit `test(frontend): CitationList renders above threshold, omits below threshold`
- [ ] 9.2 **Vitest for `ProposalActions`** Â· slice_2_sse_and_frontend Â· NEW `frontend/src/components/proposals/__tests__/ProposalActions.test.tsx` Â· 35 LoC tests Â· SCN-4, SCN-5 Â· `cd frontend && npm run test:run -- ProposalActions` passes Â· commit `test(frontend): ProposalActions dispatches decide + opens modify composer`
