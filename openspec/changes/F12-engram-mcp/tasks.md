...
- [ ] **1.1** Create `app/models/message.py` — SQLAlchemy 2.0 `Message` ORM (REQ-2). **[F12.1 · NEW · 30 LoC]** · target: REQ-2 · acceptance: `python -c "from app.models.message import Message"` exits 0 · commit: `feat(messages): add Message ORM model`.
- [ ] **1.2** Register `Message` in `app/models/__init__.py` (+1 line). **[F12.1 · MOD · 1 LoC]** · target: REQ-2 · acceptance: `Base.metadata.tables["messages"]` is present · commit: `chore(messages): register Message ORM`.
- [ ] **1.3** Create `migrations/0008_add_messages_table.sql` — idempotent `CREATE TABLE IF NOT EXISTS` + FKs + indexes (REQ-1, SCN-8). **[F12.1 · NEW · 50 LoC]** · target: REQ-1, SCN-8 · acceptance: `python migrations/run_migrations.py` runs twice with no error · commit: `feat(db): add 0008 messages table migration`.
- [ ] **1.4** Mirror the new table in `schema.sql` (+50 lines). **[F12.1 · MOD · 50 LoC]** · target: REQ-1 · acceptance: greenfield `init_db.py` creates the table · commit: `chore(db): mirror messages table in schema.sql`.
- [ ] **1.5** Create `app/core/message_store.py` — `save_message`, `list_recent`, `engram_mirror`, `_ensure_user_session` (REQ-3, REQ-6, REQ-10). **[F12.1 · NEW · 95 LoC]** · target: REQ-3, REQ-6, REQ-10 · acceptance: `pytest tests/core/test_message_store.py` passes · commit: `feat(messages): add message_store with engram mirror helper`.
- [ ] **1.6** Extend `app/core/engram_client.py` with `search`, `get_observation`, `save(topic_key)`, `delete` (+80 lines; existing methods untouched). **[F12.1 · MOD · 80 LoC]** · target: REQ-5, SCN-6 · acceptance: `engram_client.search(project=None)` raises `ValueError` · commit: `feat(engram): add search/get_observation/save(topic_key)/delete methods`.
- [ ] **1.7** Add unit tests in `tests/core/test_message_store.py` (80 LoC). **[F12.1 · NEW · 80 LoC]** · target: REQ-3, SCN-3, SCN-6 · acceptance: covers save/list/order/limit-clamp/cross-user · commit: `test(messages): add message_store unit tests`.
- [ ] **1.8** Extend `tests/test_engram_client.py` for the 4 new methods (+40 LoC). **[F12.1 · MOD · 40 LoC]** · target: REQ-5, SCN-6 · acceptance: `search(project=None)` raises `ValueError("project is required")`; `save(topic_key=…)` sends body with `topic_key` · commit: `test(engram): cover search/save/get_observation/delete`.
